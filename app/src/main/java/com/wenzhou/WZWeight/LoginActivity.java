...
public class LoginActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
			...
			public boolean onKey(View v, int keyCode, KeyEvent event) {
				...
				if (keyCode == KeyEvent.KEYCODE_ENTER) {
					...
					if (checkNetWorkStatus()) {
						new ConnectTask().execute(Constant.login);
					} else {
						showNetworkDialog();
					}
					return true;
		...
		if (index.equals("service")) {
			editText_user.setFocusable(false);
			editText_word.setFocusable(false);
			button_enter.setClickable(false);

			MyLog.d(TAG, "test 1");
			obj = new JSONObject();
			try {
				obj.put("tag", Constant.USER_LOAD);
				obj.put("user", getSharedPreferences("constant", Context.MODE_PRIVATE).getString("userName", ""));
				obj.put("password", getSharedPreferences("constant", Context.MODE_PRIVATE).getString("passWord", ""));
				MyLog.i(TAG, "obj" + obj.toString());
				MyLog.i(TAG, obj.getString("password"));
				MyLog.i(TAG, new JSONObject(obj.toString()).toString());
			} catch (JSONException e) {
			...
			if (checkNetWorkStatus()) {
				new ConnectTask().execute(Constant.serverUrl);
			} else {
				showNetworkDialog();
			}
		}
	...
	class ConnectTask extends AsyncTask<String, Integer, Boolean> {
		...
		protected Boolean doInBackground(String... params) {
			...
			try {
				...
				params1.add(new BasicNameValuePair("regionCode", "110000"));
