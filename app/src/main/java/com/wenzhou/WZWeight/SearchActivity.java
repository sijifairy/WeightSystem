...
import com.wenzhou.WZWeight.CarEditActivity.GetCarTypeTask;
import com.wenzhou.WZWeight.CarEditActivity.GetRegionTask;
import com.wenzhou.WZWeight.CarEditActivity.GetRubbishSourceTask;
...
public class SearchActivity extends Activity {
			...
			public void onClick(View v) {
				 ...
				 try {
					if(daysBetween(st,et)>93){
						Toast.makeText(SearchActivity.this, "ʱ����������С��Χ��", Toast.LENGTH_SHORT).show();
						return ;
					 }
	...
	class YesterdayWeightTask extends AsyncTask<String, Integer, Boolean> {
		...
		protected void onPreExecute() {
			...
			dialogMine.setMessage("���ڻ�ȡ����ͳ����Ϣ�����Ժ�...");
		...
		protected void onPostExecute(Boolean result) {
			...
			if (result == true) {
				...
				for (int i = 0; i < yesterdayData.size(); i++) {
					...
					str += ("\t\t" + record.get("Classname").toString()+record.get("Info").toString()+ "������ " + record.get("NumCount").toString() + " �Σ�" + "�ܾ��� "
	...
	class GetRubbishSourceTask extends AsyncTask<String, Integer, Boolean> {
			...
			try {
				...
				if (result == 1) {
					...
					try {
						...
						mRubbishSourceData.add(new SpinnerData("", "ȫ��"));
						for (int i = 0; i < resultJson.length(); i++) {
							SpinnerData tmp = new SpinnerData(((JSONObject) resultJson.get(i)).getString("ID"),
		...
		class GetPoundTask extends AsyncTask<String, Integer, Boolean> {
			...
			protected void onPreExecute() {
				super.onPreExecute();
				dialogMine.setMessage("���ڻ�ȡ�ذ�վ�б?���Ժ�...");
				if (!dialogMine.isShowing()) {
					dialogMine.show();
				}
			}
				...
				try {
					...
					if (result == 1) {
						...
						try {
							...
							mPoundsData.add(new SpinnerData("", "ȫ��"));
							for (int i = 0; i < resultJson.length(); i++) {
								SpinnerData tmp = new SpinnerData(((JSONObject) resultJson.get(i)).getString("ID"),
							...
							mDataTypeInfo.add(new SpinnerData("", "ȫ��"));
							mDataTypeInfo.add(new SpinnerData("1��", "һ��"));
							mDataTypeInfo.add(new SpinnerData("2��", "����"));
						} catch (Exception e) {
