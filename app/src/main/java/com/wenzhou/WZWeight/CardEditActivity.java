...
public class CardEditActivity extends Activity {
        ...
        savebtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (carid.equals("") || cardno.equals("") || dateend.equals("")) {
                    Toast.makeText(CardEditActivity.this, "请将IC卡信息填写完整！", Toast.LENGTH_SHORT).show();
                    return;
                }
                String poundids = "";
    ...
    class GetNoCardCarTask extends AsyncTask<String, Integer, Boolean> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            dialogMine.setMessage("正在获取未派卡车辆列表，请稍后...");
        }
    ...
    class GetCardDetailTask extends AsyncTask<String, Integer, Boolean> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            dialogMine.setMessage("正在获取IC卡信息，请稍后...");
        }
    ...
    class NewCardTask extends AsyncTask<String, Integer, Boolean> {
        ...
        protected void onPostExecute(Boolean result) {
            super.onPostExecute(result);
            if (result == true) {
                Toast.makeText(CardEditActivity.this, "新建IC卡成功！", Toast.LENGTH_SHORT).show();
                try {
                    Runtime runtime = Runtime.getRuntime();
                    runtime.exec("input keyevent " + KeyEvent.KEYCODE_BACK);
                } catch (Exception e) {
                ...
                if (saveStatus == 3) {
                    Toast.makeText(CardEditActivity.this, "卡号重复！", Toast.LENGTH_SHORT).show();
                }
            }
        }
    }
    ...
    class EditCardTask extends AsyncTask<String, Integer, Boolean> {
        ...
        protected void onPostExecute(Boolean result) {
            super.onPostExecute(result);
            if (result == true) {
                Toast.makeText(CardEditActivity.this, "修改IC卡信息成功！", Toast.LENGTH_SHORT).show();
                try {
                    Runtime runtime = Runtime.getRuntime();
                    runtime.exec("input keyevent " + KeyEvent.KEYCODE_BACK);
                } catch (Exception e) {
